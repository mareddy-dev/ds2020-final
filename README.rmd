...
- Most AI-prone platform: **`r most_ai$platform`** with **`r round(most_ai$Percentage, 1)`%** GPT-like posts.
- Least AI-prone platform: **`r least_ai$platform`** with **`r round(least_ai$Percentage, 1)`%** GPT-like posts.
- Statistical significance: **`r ifelse(chi_test_platform$p.value < 0.05, "YES", "NO")`**  
...
**How long are posts that are typically flagged as AI-generated compared to other model signatures?**
...
- Unknown posts: **`r round(mean(unknown_lengths), 1)`** characters (mean)
...
**Does AI-generated content receive different sentiment scores compared to human posts?**
...
kable(summary_results, caption = "Summary of All Findings")
```